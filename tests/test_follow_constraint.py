...
class FollowConstraintTests(unittest.TestCase):
    ...
    def test_the_final_approach_also_exempts_the_measured_face_the_handle_stands_on(self):
        ...
        def measure(role, revision):
            """Observe the role once, at its own revision, just before the move that uses it.

            Both roles in one loop is what the world model forbids: a metric update has to
            advance the revision, and a grasp pose registered ahead of the pregrasp move
            supersedes the pregrasp one.
            """
            pose = MetricPose("handle_1", role, (.58, .1, .3), (0., 0., 0., 1.), tuple([0.]*36), world.clock(),
                              "base_link", identities["entity:handle_1"]+revision, identities["calibration_id"],
            ...
            return {"target": {"entity_id": "handle_1", "pose_role": role}, "profile_id": "bench_transit"}

        # Standing off is not contact: nothing is exempt on the way in.
        args = measure("pregrasp", 1)
